...
import { awardStar, loadProfiles, makeDeck, matchCards, type Profile } from './game'
...
export default function App() {
  ...
  const choose = (id:string) => {
    ...
    if(next.length===2){
      ...
      if(a?.value===b?.value){
        setDeck(cards=>matchCards(cards,next).deck); setOpen([]); setMessage(`Rätt! ${a?.value} och ${b?.value}`)
        setProfiles(items=>{const updated=awardStar(items,active.id); localStorage.setItem('mega-profiles',JSON.stringify(updated)); return updated})
  ...
  return <main>
    ...
    <section className="game" aria-label="Memory med tal">{deck.map(card=>{const shown=card.matched||open.includes(card.id); return <button data-testid={card.id} aria-label={shown?`Tal ${card.value}`:'Dolt kort'} disabled={card.matched} onClick={()=>choose(card.id)} className={shown?'card shown':'card'} key={card.id}>{shown?card.value:'✦'}</button>})}</section>
