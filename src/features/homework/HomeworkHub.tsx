...
export function HomeworkHub(){
  ...
  return <section className="homework-hub">
    <section className="track-picker" aria-label="Välj ämne">{homeworkSubjects.map(item=><button className={item.id===subjectId?'track active':'track'} onClick={()=>regenerate(item.id,stage)} key={item.id}><span>{item.shortName}</span><b>{item.name}</b></button>)}</section>
    <section className="hero"><div><p className="eyebrow">{stageLabel[stage].toUpperCase()}</p><h2>Läxa</h2><div className="stage-toggle"><button className={stage==='forskola'?'active':''} onClick={()=>regenerate(subjectId,'forskola')}>Förskola</button><button className={stage==='ak1'?'active':''} onClick={()=>regenerate(subjectId,'ak1')}>Åk 1</button></div></div><div className="actions"><button className="level" onClick={()=>setShowAnswers(value=>!value)}>{showAnswers?'Dölj facit':'Visa facit'}</button><button className="reset" onClick={()=>regenerate()}>Nytt blad</button></div></section>
