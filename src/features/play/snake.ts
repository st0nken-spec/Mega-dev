...
export const nextTarget=(score:number)=>score%9+1
export const createSnake=(food:Point={x:4,y:2}):SnakeState=>({snake:[{x:2,y:2},{x:1,y:2}],direction:'right',food,score:0,over:false,target:1})
...
export const stepSnake=(state:SnakeState,random:()=>number=Math.random,size=6):SnakeState=>{
  ...
  const snake=[next,...body];const score=state.score+(ate?1:0);return{...state,snake,score,target:ate?nextTarget(score):state.target,food:ate?placeFood(snake,random,size):state.food}
}
