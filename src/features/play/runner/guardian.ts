...
export type GuardianState = { hits: number; attack: GuardianAttack; phase: GuardianPhase; ticksLeft: number }
...
export const createGuardian = (random: () => number = Math.random): GuardianState => ({ hits: 0, attack: nextAttack(random), phase: 'telegraph', ticksLeft: TELEGRAPH_TICKS })
...
export const tickGuardian = (state: GuardianState, lane: Lane, action: Action, random: () => number = Math.random): GuardianStep => {
  ...
  return { state: { ...state, attack: nextAttack(random), ticksLeft: TELEGRAPH_TICKS }, outcome: 'hit-player' }
}
...
export const landHit = (state: GuardianState, random: () => number = Math.random): GuardianStep => {
  ...
  return { state: { hits, attack: nextAttack(random), phase: 'telegraph', ticksLeft: TELEGRAPH_TICKS }, outcome: 'ongoing' }
}

export const missShield = (state: GuardianState, random: () => number = Math.random): GuardianState => ({ ...state, attack: nextAttack(random), phase: 'telegraph', ticksLeft: TELEGRAPH_TICKS })
